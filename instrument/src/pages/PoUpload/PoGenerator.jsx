...
function PoGenerator() {
    ...
    const handleSubmit = async () => {
        if (selectedIndex === null) {
            setSnackbar({ open: true, message: 'Please select a purchase order', severity: 'error' });
            return;
        }
        if (!selectedFile) {
        ...
        try {
            const result = await dispatch(postFileUpload({
                quotationId: selectedPo._id,
                formData: formData
              })).unwrap();
              
    
            setSnackbar({ open: true, message: result.message || 'File uploaded successfully', severity: 'success' });
            ...
            setSnackbar({ open: true, message: error || 'Failed to upload file', severity: 'error' });
        }
    };
                        ...
                        <div className='border border-gray-600 rounded-lg h-[35vh] overflow-scroll'>
                            {uploadPo.length === 0 ? (
                                ...
                                uploadPo.map((row, index) => (
                                    ...
                                    >
                                        <div
                                            className={`min-w-[700px] flex justify-between items-center rounded-md gap-4 px-5 py-3 border-b ${selectedIndex === index ? 'bg-yellow-100' : 'bg-gray-100'} hover:bg-gray-100 transition-all duration-200`}
                                        >
                                            <input
                                                type="radio"
                                                name='po selected'
                                            ...
                                            />
                                            <p className='w-[20%] text-gray-700 font-medium'>{row.quotationNumber}</p>
                                            <p className='w-[20%] text-gray-700 font-medium'>{new Date(row.createdAt).toLocaleDateString()}</p>
                                            <p className='w-[100%] text-gray-600 text-sm'>{row.problemDescription}</p>
                                        </div>
                                    </div>
                                ))
                            )}
